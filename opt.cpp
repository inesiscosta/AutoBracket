...
int main() {
  ...
  std::vector<std::vector<std::tuple<std::string, int, int>>> dp(m, std::vector<std::tuple<std::string, int, int>>(m, std::make_tuple("", INT_MAX, -1)));

  for (int i = 0; i < m; ++i) {
    std::cin >> sequence[i];
    dp[i][i] = std::make_tuple(std::to_string(sequence[i]), 0, sequence[i]);
  }
